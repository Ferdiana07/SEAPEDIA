...
## 4.8 Penjelasan Lengkap: Services, Stores & Communication Flows

### Kenapa Butuh Services & Stores di Frontend?

#### Masalah Tanpa Services & Stores

```
┌─────────────────────────────────────────────────────────────────┐
│         TANPA SERVICES & STORES - RIBET! ❌                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  LoginPage.jsx                                                  │
│  ├── copy-paste axios langsung di sini                         │
│  ├── copy-paste headers config                                 │
│  ├── copy-paste error handling                                  │
│  └── copy-paste token logic                                    │
│                                                                  │
│  RegisterPage.jsx                                               │
│  ├── copy-paste axios lagi                                     │
│  ├── copy-paste headers config lagi                            │
│  ├── copy-paste error handling lagi                            │
│  └── copy-paste token logic lagi                               │
│                                                                  │
│  HomePage.jsx                                                   │
│  ├── copy-paste axios LAGI                                     │
│  └── ... belum selesai...                                       │
...
│  ❌ GAGAL DI SATU TEMPAT = GAGAL DI SEMUA!                    │
│  ❌ SUSAH DIKELOLA!                                            │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

#### Dengan Services & Stores - RAPIH! ✅
...
│  │  │─────────────│  │─────────────│  │─────────────│    │   │
│  │  │ • login()  │  │ • getAll()  │  │ • addItem()│    │   │
│  │  │ • register │  │ • getById() │  │ • checkout()│    │   │
│  │  │ • logout() │  │ • create()  │  │ • getCart()│    │   │
│  │  └─────────────┘  └─────────────┘  └─────────────┘    │   │
│  │                                                         │   │
│  │  KEUNTUNGAN:                                            │   │
│  │  • Kode centralized di satu tempat                      │   │
...
│  │  • Tidak perlu pass props berkali-kali                   │   │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1️⃣  USER MEMBUKA HALAMAN LOGIN                                           │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Browser                                                              │
│          │                                                                  │
...
│    │            │                                                        │
│    │ ┌───────┐ │                                                        │
│    │ │ Email │ │  ← User ketik email                                     │
│    │ └───────┘ │                                                        │
│    │ ┌───────┐ │                                                        │
│    │ │Password│ │  ← User ketik password                                  │
│    │ └───────┘ │                                                        │
│    │ [ LOGIN ] │  ← User klik tombol                                      │
│    └───────┬─────┘                                                        │
│            │ handleSubmit()                                               │
│            ▼                                                              │
│                                                                              │
│  2️⃣  FRONTEND KIRIM REQUEST KE BACKEND                                   │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
...
│            │   {                                                         │
│            │     success: true,                                         │
│            │     user: { id: 1, name: "Budi", roles: [...] },         │
│            │     token: "1|abc123xyz..."                               │
│            │   }                                                         │
│            │                                                              │
│  3️⃣  FRONTEND SIMPAN DATA & UPDATE UI                                │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Frontend (React)                                                      │
│            │                                                               │
...
│    │  └───────────────────────────────────┘ │                            │
│    │                                              │                            │
│    │  Persist → localStorage['seapedia-auth']│                            │
│    └─────────────────────────────────────────┘                            │
│            │                                                               │
│            │  Navbar otomatis update!                                     │
│            ▼                                                               │
│    ┌─────────────┐      ┌─────────────┐      ┌─────────────┐              │
│    │   NAVBAR    │      │   NAVBAR    │      │   NAVBAR    │              │
│    │            │      │            │      │            │              │
│    │ [Login]    │  →   │ [Avatar] B │  →   │ 🛒 Cart(3) │              │
│    │ [Register] │      │ Buyer      │      │Dropdown    │              │
│    │            │      │ ▼          │      │▼          │              │
│    │ (BELUM     │      │ • Profile │      │ • Profile │              │
│    │  LOGIN)   │      │ • Orders  │      │ • Orders  │              │
│    │            │      │ • Logout  │      │ • Logout  │              │
│    └─────────────┘      └─────────────┘      └─────────────┘              │
│                                                                              │
...
#### Kode Login di Component

```javascript
// Di LoginPage.jsx

const LoginPage = () => {
  // Ambil fungsi dari stores
  const { setAuth } = useAuthStore()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Panggil API via service
      const response = await authService.login(formData)
      
      // 2. Jika berhasil, simpan ke store
      if (response.success) {
        setAuth(response.user, response.token)
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1️⃣  USER KLIK TOMBOL "TAMBAH KE CART"                                  │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       ProductDetailPage.jsx                                                │
│            │                                                                 │
...
│    │           cartStore (Zustand)           │                             │
│    │                                            │                             │
│    │  State BEFORE (cart kosong):             │                             │
│    │  items: []                               │                             │
│    │  store: null                            │                             │
│    │                                            │                             │
│    │  State AFTER (setelah add):              │                             │
│    │  items: [{                               │                             │
│    │    product_id: 1,                       │                             │
│    │    name: "Nasi Goreng",                  │                             │
│    │    price: 25000,                        │                             │
...
│    └─────────────────────────────────────────┘                             │
│            │                                                                 │
│            │  SEMUA component yang pakai cartStore AUTO-UPDATE!            │
│            ▼                                                                 │
│       ┌─────────────┐       ┌─────────────┐       ┌─────────────┐        │
│       │   NAVBAR    │       │   CART     │       │   FOOTER   │        │
│       │            │       │   PAGE     │       │            │        │
│       │ 🛒 Cart(1)│       │ Item: 1   │       │ Total: 25k │        │
│       │  ↑ UPDATE! │       │ Total: 25k│       │  ↑ UPDATE! │        │
│       └─────────────┘       └─────────────┘       └─────────────┘        │
│                                                                              │
│  2️⃣  SYARAT: SINGLE-STORE RULE                                          │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│  ⚠️ PERATURAN: Cart hanya boleh berisi produk dari 1 TOKO!               │
│                                                                              │
│  Jika user mau tambah produk dari toko BERBEDA:                              │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────┐              │
│  │                                                         │              │
│  │  addItem(productFromStoreB)                            │              │
│  │                                                         │              │
│  │  // Cek store                                          │              │
│  │  store: { id: 1, name: "Dapur Budi" }  ← Store lama │              │
│  │  if (store.id !== product.store_id) {                 │              │
│  │    // Store BERBEDA!                                   │              │
│  │    items: []  ← KOSONGKAN CART!                       │              │
│  │  }                                                     │              │
│  │                                                         │              │
│  │  // Tambah produk baru                                 │              │
│  │  items: [{ productFromStoreB }]                        │              │
│  │  store: { id: 2, name: "Warung Ani" }  ← Store baru │              │
│  │                                                         │              │
│  │  ⚠️ PERINGATAN: Item dari "Dapur Budi" dihapus!     │              │
│  │  User harus checkout atau kehilangan item sebelumnya!   │              │
│  │                                                         │              │
│  └─────────────────────────────────────────────────────────┘              │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
#### Kode Add to Cart di Component

```javascript
// Di ProductDetailPage.jsx

const ProductDetailPage = () => {
  const { addItem } = useCartStore()
  const { success, warning } = useUIStore()
  
  const product = { /* data dari API */ }
  
  const handleAddToCart = () => {
    const cart = useCartStore.getState()
    
    // Cek single-store rule
    if (cart.store && cart.store.id !== product.store_id) {
      warning('Cart dihapus! Kamu hanya bisa belanja dari 1 toko.')
    }
    
    addItem(product, 1)
    success('Produk ditambahkan ke cart!')
  }
}
```
...
### Alur 3: Register Flow

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         ALUR REGISTER LENGKAP                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  1️⃣  USER MENGISI FORM REGISTER                                           │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Browser                                                              │
│          │                                                                 │
...
│    │RegisterPage│                                                        │
│    │            │                                                        │
│    │ Name: Budi │  ← Input nama                                          │
│    │ Email: ... │  ← Input email                                         │
│    │ Password:  │  ← Input password                                       │
│    │ Confirm:   │  ← Konfirmasi password                                   │
│    │ [REGISTER] │  ← Klik tombol                                          │
│    └─────────────┘                                                        │
│            │ handleSubmit()                                               │
│            ▼                                                              │
│                                                                              │
│  2️⃣  FRONTEND KIRIM REQUEST                                              │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Frontend (React)               Backend (Laravel)                      │
│            │                                    │                           │
...
│       Frontend (React)                                                      │
│            │                                                               │
│            │  success('Registrasi berhasil!')  ← Toast notification        │
│            │  navigate('/login')             ← Redirect ke login           │
│            ▼                                                               │
│    ┌─────────────┐                                                        │
│    │  LoginPage │  ← User harus login manually                              │
│    └─────────────┘                                                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
### Alur 4: Request dengan Token (Authenticated Request)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│              ALUR REQUEST DENGAN TOKEN (SETELAH LOGIN)                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  SAAT USER Sudah LOGIN, SETIAP REQUEST HARUS PAKAI TOKEN                   │
│                                                                              │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
...
│       CartPage.jsx                                                          │
│            │                                                                 │
│            │  // Ambil data dari cartStore                                  │
│            │  const { items, store, getTotalPrice } = useCartStore()       │
│            │                                                                 │
│            ▼                                                                 │
│    ┌─────────────────────────────────────────┐                             │
│    │           CART STORE (Zustand)           │                             │
...
│            │    items: [...],                                             │
│            │    address_id: 1,                                            │
│            │    shipping_address: "Jl. Mawar..."                         │
│            │  }                                                           │
│            ▼                                                                 │
│                                                                              │
│  3️⃣  BACKEND PROSES CHECKOUT                                              │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Backend (Laravel)                                                     │
│            │                                                                 │
...
│    │                                         │                             │
│    │  1. Validasi:                        │                             │
│    │     • Cek wallet balance >= total     │                             │
│    │     • Cek stock produk cukup         │                             │
│    │     • Cek address valid               │                             │
│    │                                         │                             │
│    │  2. DB Transaction:                   │                             │
│    │     BEGIN TRANSACTION                  │                             │
│    │     • INSERT orders                  │                             │
│    │     • INSERT order_items             │                             │
│    │     • UPDATE products (kurangi stock)│                             │
│    │     • UPDATE wallets (debit balance) │                             │
│    │     • INSERT transactions            │                             │
│    │     COMMIT TRANSACTION                │                             │
│    │                                         │                             │
│    └─────────────────────────────────────────┘                             │
│                                                                              │
│  4️⃣  FRONTEND UPDATE UI                                                  │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Frontend (React)                                                      │
│            │                                                                 │
...
│            │  navigate('/buyer/orders')  ← Ke halaman orders                │
│            │                                                                 │
│            │  // Jika gagal:                                               │
...
│            ▼                                                                 │
│    ┌─────────────┐                                                        │
│    │ OrdersPage │  ← Pesanan muncul di sini                                │
│    └─────────────┘                                                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  SEBELUMNYA: User Budi sudah login sebagai BUYER                           │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────┐              │
│  │  authStore:                                           │              │
│  │    user: { name: "Budi", roles: ["buyer", "seller"] }│              │
│  │    activeRole: "buyer"                              │              │
...
│  ┌─────────────┐                                                        │
│  │ [Avatar] B │                                                        │
│  │ Badge: Buyer│  ← Tampilkan role Buyer                                │
│  │ ▼           │                                                        │
│  │ • Profile  │                                                        │
│  │ • Orders   │  ← Link Orders Buyer                                   │
│  │ • Seller? │  ← Tidak terlihat (bukan Seller aktif)                 │
│  │ • Logout   │                                                        │
│  └─────────────┘                                                        │
│                                                                              │
│  1️⃣  USER KLIK DROPDOWN & PILIH "Switch to Seller"                       │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Navbar.jsx                                                           │
│            │                                                                 │
...
│            │                                                                 │
│            │  // Jika role belum ada:                                        │
│            │  await authService.assignRole('seller')  ← Ambil role dulu    │
│            │                                                                 │
│            │  // Pilih role                                                  │
│            │  await authService.selectRole('seller')  ← Aktifkan role       │
│            │                                                                 │
│            ▼                                                                 │
│                                                                              │
│  2️⃣  BACKEND PROSES                                                      │
...
│            │                                                                 │
│            │  // assignRole:                                                │
│            │  INSERT user_roles (role: 'seller')                           │
│            │                                                                 │
│            │  // selectRole:                                                │
│            │  UPDATE user_roles SET is_active = false WHERE user_id = 1    │
│            │  UPDATE user_roles SET is_active = true WHERE role = 'seller' │
│            │                                                                 │
│            ▼                                                                 │
│    ┌─────────────────────────────────────────┐                             │
│    │  Response:                              │                             │
...
│    └─────────────────────────────────────────┘                             │
│                                                                              │
│  3️⃣  FRONTEND UPDATE STORE & UI                                           │
│  ─────────────────────────────────────────────────────────────────────────   │
│                                                                              │
│       Frontend (React)                                                     │
│            │                                                                 │
│            │  // Update store                                              │
│            │  updateUser(response.user)  ← Update data user                │
│            │  setActiveRole('seller')  ← Update role aktif                │
│            │                                                                 │
│            ▼                                                                 │
│    ┌─────────────────────────────────────────┐                             │
│    │  authStore (BARU):                    │                             │
...
│  ┌─────────────┐                                                        │
│  │ [Avatar] B │                                                        │
│  │ Badge: Seller│  ← Tampilkan role Seller                             │
│  │ ▼           │                                                        │
│  │ • Profile  │                                                        │
│  │ • Orders   │                                                        │
│  │ • Seller Dashboard│  ← Muncul link Seller!                          │
│  │ • Switch to Buyer│  ← Muncul opsi switch back                       │
│  │ • Logout   │                                                        │
│  └─────────────┘                                                        │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
...
### Diagram Komprehensif: Seluruh Arsitektur

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        ARSITEKTUR LENGKAP SEAPEDIA                              │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│     ┌──────────────────────────────────────────────────────────────────────┐   │
│     │                         USER'S BROWSER                                  │   │
...
│     │  │  │                     PAGES                                │  │ │   │
│     │  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │  │ │   │
│     │  │  │  │HomePage │ │LoginPage │ │CartPage  │ │Dashboard │   │  │ │   │
│     │  │  │  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘   │  │ │   │
│     │  │  └───────┼───────────┼───────────┼───────────┼──────────┘  │ │   │
│     │  │          │           │           │           │              │  │ │   │
│     │  │          └───────────┴─────┬─────┴───────────┘              │  │ │   │
...
│     │  │  POST /api/cart/items      → CartController@store             │ │   │
│     │  │  POST /api/orders          → OrderController@checkout         │ │   │
│     │  └────────────────────────────────────────────────────────────────┘ │   │
│     │                              │                                        │
│     │                              ▼                                        │
│     │  ┌────────────────────────────────────────────────────────────────┐ │   │
...
| Aspek | Backend (Laravel) | Frontend (React) |
|-------|-------------------|-----------------|
| **Fungsi** | Logic aplikasi, database | Tampilan, interaksi user |
| **Services** | - | Fungsi untuk PANGGIL API Backend |
| **Stores** | - | Penyimpanan data sementara (client-side) |
| **Auth** | Generate token, validasi | Simpan token, cek login |
| **Data** | MySQL Database | localStorage (via Zustand persist) |
| **API** | Endpoint: `/api/auth/login` | Fungsi: `authService.login()` |
| **Token** | Buat & return token | Terima & simpan token |

---

### Alur Request API: Tanpa vs Dengan Services

```
┌─────────────────────────────────────────────────────────────────┐
│  TANPA SERVICES (TIDAK DISARANKAN) ❌                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  // Di LoginPage.jsx                                            │
...
│      // Handle response...                                       │
│    } catch (err) {                                             │
│      // Handle error lagi... (DUPLIKAT!)                        │
│    }                                                           │
│  }                                                             │
│                                                                  │
│  ❌ Kode axios diulang-ulang!                                  │
│  ❌ Error handling tidak konsisten!                            │
│  ❌ Token handling manual di setiap tempat!                     │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  DENGAN SERVICES (DISARANKAN) ✅                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  // Di authService.js (SATU TEMPAT)                             │
│  const authService = {                                          │
│    login: async (data) => {                                    │
│      const response = await api.post('/auth/login', data)      │
│      return response.data                                      │
...
│      // Handle response...                                       │
│    } catch (err) {                                             │
│      // Error handling centralized di api.js interceptor!      │
│    }                                                           │
│  }                                                             │
│                                                                  │
│  // Di RegisterPage.jsx                                        │
...
│  }                                                             │
│                                                                  │
│  ✅ Kode axios di satu tempat!                                 │
│  ✅ Error handling konsisten!                                  │
│  ✅ Token handling otomatis via interceptor!                  │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ✅ Communication Flows (Login, AddToCart, Register, dll)     │
│                                                                  │
